 ...
 * @since 1.2.2
 */
function xd_add_help_text( $contextual_help, $screen_id, $screen ) {
	...
	if ( 'xdmsg_page_dictionary_page' == $screen->id ) {
		$about_infos =
		'<p>' . /* translators: */ sprintf( __( 'xili-dictionary is a plugin (compatible with xili-language) to build a multilingual dictionary saved in the post tables of WordPress as custom post type (%s). With this dictionary, it is possible to create and update .mo file in the current theme folder. And more...', 'xili-dictionary' ), '<em>' . $xili_dictionary->xdmsg . '</em>' ) .
		'</p>' .
		...
		// 2.3.8
		$wikilink = $xili_dictionary->wikilink . '/index.php/Xili-language:_languages_list_insertion_in_nav_menu';
		$to_remember =
		...
		$wikilink = $xili_dictionary->wikilink;
		$to_remember =
		...
		if ( isset( $_GET['scan'] ) ) {
			...
			'</ul>' .
			/* translators: */
			'<p>' . sprintf( __( '<a href="%s" target="_blank">Xili Wiki Documentation</a>', 'xili-dictionary' ), $xili_dictionary->wikilink ) . '</p>';
 ...
 *
 */
function xd_localize_admin_js( $case_news, $news_id, $xili_dictionary ) {
